...
function login() {
  axios
    .post("https://api.phuonghight.click/auth/login", data)
    .then((res) => {
...
function register() {
  axios
    .post("https://api.phuonghight.click/auth/register", data)
    .then((res) => {
      console.log(res);
    ...
    .catch((error) => {
      ...
      if (
        ...
        `User with ${data.email} is already exist`
      )
  ...
  <div>
    ...
    <div v-if="loginForm && loginHome" class="login form-wrap center">
      ...
      <form action="">
        ...
        />
        <br />
        <p v-if="!isLoginFail" class="form-error">
          Login fail! Pls try again or register
        </p>
        <div class="change-register">
          <p class="form-feature">Change password?</p>
          <p @click="loginForm = false" class="form-feature">
